...
\end{lstlisting}
Listing \ref{lst:clientHandshake}, that is send by the client, is a pretty standard HTTP request. It is build with multiple headers, some of which are mandotory for a request to be valid. If one of the headers is not understood, the server will reply with "400 Bad Request" and it will close the socket afterwards \cite{BadRequest}. In some cases, it will also give a reason why handshake failed, although browser do not display these messages. If there is 



\begin{figure}[h]
\centering
\includegraphics[scale=0.35]{images/server.pdf}
