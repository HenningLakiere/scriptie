...
%\paragraph{OpenCL programming model.} OpenCL programs are executed on devices containing CPUs, GPUs or other compute accelerators. A device can consist of one or more of these compute units that are all attached to a host processor like in figure \ref{fig:OpenCLDevices}. The programs are written in a language that is understandable by all devices that are supported by OpenCL. This allows developers to accelerate their programs by writing task- or data-parallel computations \cite{stone2010opencl}. A programming language based on C is used to code kernels. These kernels are the functions that are gonna be executed on the devices. As mentioned earlier, OpenCL provide APIs that allow you to run the program on a host device that will launch the kernels on the compute devices that are connected with this host. It also allows to manage the distribution of the devices memory. The next paragraph describes the differents sorts of memory available.

%\paragraph{Memory model.} Managing the device's memory usage is very useful when it comes down to improve the speed and efficiency of all kernels. Some memory types are faster than others while some contain more space to work with. The memory in OpenCL is split up in 2 main parts
