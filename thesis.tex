...
The most important components of the ws or wss are the host and its port. the host is determined by the servers ip address. While the port defines which port the server uses for the communication. If there is no specified port, the standard port for ws is 80, and the standard port for wss is 443
